...
| Характеристика | Local LLM (Llama-3.1-8B via Ollama) | YandexGPT 5.1 Pro (Cloud) | OpenAI GPT-5 (Cloud) |
| :--- | :--- | :--- | :--- |
| **Качество ответов** | Среднее/Высокое. Хорошо понимает контекст, возможны галлюцинации на специфичных терминах. | Высокое. Отлично работает с русским языком. По внутренним тестам Яндекса превосходит GPT-4.1 в 56% случаев. | Эталонное. Лучшее понимание сложных инструкций. Встроенный режим reasoning. |
...
| **Стоимость** | **0₽** (локальные ресурсы). | ~0.2₽/1K токенов. Есть Free Tier и гранты. | ~$1.25/1M input + $10/1M output токенов. Сложности с оплатой из РФ. |
...
| Характеристика | sentence-transformers/all-MiniLM-L6-v2 | intfloat/multilingual-e5-large | DeepPavlov/rubert-base-cased-sentence | OpenAI text-embedding-3-large |
| :--- | :--- | :--- | :--- | :--- |
| **Тип** | Локальная (HuggingFace) | Локальная (HuggingFace) | Локальная (HuggingFace) | Облачная (API) |
| **Размер эмбеддинга** | 384 | 1024 | 768 | 3072 (настраивается через параметр dimensions) |
| **Скорость индексации** | ~500 doc/сек (CPU) | ~100 doc/сек (CPU) | ~150 doc/сек (CPU) | ~300 doc/сек (API) |
| **Качество поиска (рус.)** | Среднее (оптимизирована для EN) | **Высокое** (MTEB: top-5 для RU) | Высокое (специализация на RU) | Высокое (MTEB: 64.6%, MIRACL: 54.9%) |
| **Стоимость** | 0₽ | 0₽ | 0₽ | ~$0.13/1M токенов |
...
| `docker-compose.yml` | Оркестрация: консольный бот + Telegram-бот + Ollama |
| `requirements.txt` | Python-зависимости |

### Запуск

```bash
# 1. Указать токен Telegram-бота в docker-compose.yml
#    (заменить YOUR_TELEGRAM_BOT_TOKEN_HERE на реальный токен)

# 2. Собрать и запустить
docker compose up -d

# 3. Скачать модель (только при первом запуске)
docker exec -it rag-ollama ollama pull llama3.1

# 4. Подключиться к консольному боту
docker attach rag-bot
# (выход: Ctrl+P, Ctrl+Q — без остановки контейнера)

# 5. Telegram-бот запускается автоматически
```
...
docker compose down -v --rmi local  # полная очистка (образы + volumes)
```
...
Оба бота используют один Docker-образ (`rag-app:latest`). Консольный бот запускается через `rag_bot.py`, Telegram-бот — через `telegram_bot.py`. Ollama доступна по внутренней сети Docker (`http://ollama:11434`). Логи сохраняются через volume на хост.
